ovreGO <- function(genes, allgenes, ontology="BP", algorithm="parentchild", statistic="fisher", nterm=100, plot=FALSE, nword=30, min.freq=1, scale=c(2,0.5), ...){
  geneList <- factor(as.integer(allgenes %in% genes))
  names(geneList) <- allgenes
  godata <- new("topGOdata", ontology=ontology, allGenes=geneList, annot = annFUN.org, mapping="org.Hs.eg.db")
